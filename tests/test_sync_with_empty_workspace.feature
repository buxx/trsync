Feature: SynchronizeWithWorkspace
    ...
    Scenario: Synchronize from existing empty workspace and create files and folders on local
        ...
        And In workspace "MyWorskpace1", create local file "/toto.txt" with content "toto"
        And In workspace "MyWorskpace1", create local folder at "/MyFolder"
        And In workspace "MyWorskpace1", create local file "/MyFolder/toto2.txt" with content "toto2"

        Then In workspace "MyWorskpace1", I should see remote file "/toto.txt"
        And In workspace "MyWorskpace1", I should see remote folder at "/MyFolder"
        And In workspace "MyWorskpace1", I should see remote file "/MyFolder/toto2.txt"
    ...
    Scenario: Synchronize from existing empty workspace and create then remove file in repetition on remote
        ...
        Then In workspace "MyWorskpace1", I should not see local file "/toto.txt" during 10 seconds
